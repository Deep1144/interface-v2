...
const LeaderboardWrapper: React.FC<Props> = (props) => {
  const { showPastWinners = false } = props;
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_LEADERBOARD_APP_URL}/${
          showPastWinners ? 'last-compitition-winner' : 'leaderboard'
        }?pool=${contestFilter.address}&days=${durationIndex}`,
      );
      if (!res.ok) {
